...
@main.command("tools")
def list_tools() -> None:
    click.echo(json.dumps({"ok": True, "version": "v1", "data": TOOL_REGISTRY.get("tools", {})}, indent=2))


@main.command()
@click.argument('prompt')
@click.option('--hold', is_flag=True, help='Keep gateway alive after success until Ctrl+C')
@click.option('--lang', type=click.Choice(['python','node','nextjs','go','rust','java','.net','cpp']), default='python', help='Language runner to use')
def run(prompt: str, hold: bool, lang: str) -> None:
    ...
    if passed:
        ...
        click.echo("❌ FAIL")
    click.echo(json.dumps({"plan": plan, "start": start, "check": check, "report": report}, indent=2))
...
def export(task_id: str, dest: str) -> None:
    ...
    if not src.exists():
        click.echo(json.dumps({"ok": False, "version": "v1", "error": {"code": "E_ARGS", "message": "unknown task_id"}}))
        sys.exit(1)
    dest_path = Path(dest)
    ...
    for p in src.rglob('*'):
        if p.is_file():
            ...
            shutil.copy2(p, out)
    click.echo(json.dumps({"ok": True, "version": "v1", "data": {"exported": task_id, "dest": str(dest_path)}}))
