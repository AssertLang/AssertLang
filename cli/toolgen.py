...
func Handle(req map[string]interface{}) map[string]interface{} {
    ...
    fmtVal, _ := req["format"].(string)
    if fmtVal != "json" {
        return errResp("E_UNSUPPORTED", "unsupported format: "+fmtVal)
    }
    schemaObj, ok := req["schema"]
    if !ok || schemaObj == nil {
        return errResp("E_ARGS", "schema must be an object and content must be a string")
    }
    ...
    if err != nil {
        return errResp("E_SCHEMA", err.Error())
    }
    var data interface{}
    if err := json.Unmarshal([]byte(content), &data); err != nil {
        return okResp(map[string]interface{}{"valid": false, "issues": []interface{}{"json decode failed: " + err.Error()}})
    }
    ...
    if result.Valid() {
        return okResp(map[string]interface{}{"valid": true, "issues": []interface{}{} })
    }
    issues := make([]interface{}, 0, len(result.Errors()))
    for _, verr := range result.Errors() {
        issues = append(issues, verr.String())
    }
    return okResp(map[string]interface{}{"valid": false, "issues": issues})
}
...
"""
    if key == ("rust", "json_validator"):
        return """use jsonschema::{Draft, JSONSchema};
use serde_json::{json, Value};
...
pub fn handle(request: &Value) -> Value {
    ...
    if fmt != "json" {
        return json!({ "ok": false, "version": VERSION, "error": { "code": "E_UNSUPPORTED", "message": format!("unsupported format: {}", fmt) } });
    }
    let schema = match request.get("schema") {
        Some(Value::Object(_)) => request.get("schema").unwrap(),
        _ => return json!({ "ok": false, "version": VERSION, "error": { "code": "E_ARGS", "message": "schema must be an object and content must be a string" } }),
    };
    let content = match request.get("content").and_then(Value::as_str) {
        Some(value) => value,
        None => return json!({ "ok": false, "version": VERSION, "error": { "code": "E_ARGS", "message": "schema must be an object and content must be a string" } }),
    };
    let data: Value = match serde_json::from_str(content) {
        Ok(value) => value,
        Err(err) => {
            return json!({ "ok": true, "version": VERSION, "data": { "valid": false, "issues": [format!("json decode failed: {}", err)] } });
        }
    };
    let compiled = match JSONSchema::options().with_draft(Draft::Draft7).compile(schema) {
        Ok(schema) => schema,
        Err(err) => {
            return json!({ "ok": false, "version": VERSION, "error": { "code": "E_SCHEMA", "message": err.to_string() } });
        }
    };
    if let Err(errors) = compiled.validate(&data) {
        let issues: Vec<String> = errors.map(|e| e.to_string()).collect();
        return json!({ "ok": true, "version": VERSION, "data": { "valid": false, "issues": issues } });
    }
    json!({ "ok": true, "version": VERSION, "data": { "valid": true, "issues": [] } })
}
"""
    if key == ("dotnet", "json_validator"):
        return """using System;
using System.Collections.Generic;
...
public static class Adapter {
  ...
  public static Dictionary<string, object> Handle(Dictionary<string, object> request) {
    ...
    request.TryGetValue("format", out var formatObj);
    var format = formatObj?.ToString();
    if (!string.Equals(format, "json", StringComparison.OrdinalIgnoreCase)) {
      return Error("E_UNSUPPORTED", $"unsupported format: {format}");
    }
    if (!request.TryGetValue("schema", out var schemaObj) || schemaObj is null || !request.TryGetValue("content", out var contentObj) || contentObj is not string content) {
      return Error("E_ARGS", "schema must be an object and content must be a string");
    }
    string schemaJson;
    try {
      ...
      return Error("E_SCHEMA", ex.Message);
    }
    JsonSchema schema;
    try {
...
def handle(request: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        return error('E_UNSUPPORTED', f'unsupported encoding: {exc}')
    except UnicodeDecodeError as exc:
        return error('E_RUNTIME', f'unicode decode error: {exc}')
    return ok({'content': content})
"""
    if key == ("node", "file_reader"):
...
function handle(request) {
  ...
  const encoding = typeof request.encoding === 'string' && request.encoding.trim() ? request.encoding : 'utf-8';
  try {
    const content = fs.readFileSync(filePath, { encoding });
    return ok({ content });
  } catch (err) {
    if (err && err.code === 'ENOENT') return error('E_RUNTIME', 'file not found');
    return error('E_RUNTIME', String(err && err.message ? err.message : err));
  }
}

module.exports = { VERSION, handle };
"""
    if key == ("go", "file_reader"):
        return """package main
...
func Handle(req map[string]interface{}) map[string]interface{} {
    ...
    if src, _ := req["source"].(string); src != "file" {
        return errResp("E_UNSUPPORTED", "only file source supported")
    }
    path, ok := req["path"].(string)
    if !ok || path == "" {
        return errResp("E_ARGS", "path must be a string")
    }
    if enc, ok := req["encoding"].(string); ok && enc != "" && enc != "utf-8" && enc != "utf8" {
        return errResp("E_UNSUPPORTED", "only utf-8 encoding supported")
    }
    data, err := os.ReadFile(path)
    if err != nil {
        if errors.Is(err, os.ErrNotExist) {
            return errResp("E_RUNTIME", "file not found")
        }
        return errResp("E_RUNTIME", err.Error())
    }
    return okResp(map[string]interface{}{"content": string(data)})
}
...
pub fn handle(request: &Value) -> Value {
    ...
    if source != "file" {
        return json!({ "ok": false, "version": VERSION, "error": { "code": "E_UNSUPPORTED", "message": "only file source supported" } });
    }
    let path = match request.get("path").and_then(Value::as_str) {
        Some(value) if !value.is_empty() => value,
        _ => return json!({ "ok": false, "version": VERSION, "error": { "code": "E_ARGS", "message": "path must be a string" } }),
    };
    if let Some(encoding) = request.get("encoding").and_then(Value::as_str) {
        let lower = encoding.to_ascii_lowercase();
        if lower != "utf-8" && lower != "utf8" {
            return json!({ "ok": false, "version": VERSION, "error": { "code": "E_UNSUPPORTED", "message": format!("unsupported encoding: {}", encoding) } });
        }
    }
    match fs::read_to_string(path) {
        Ok(content) => json!({ "ok": true, "version": VERSION, "data": { "content": content } }),
        Err(err) => {
            if err.kind() == io::ErrorKind::NotFound {
                json!({ "ok": false, "version": VERSION, "error": { "code": "E_RUNTIME", "message": "file not found" } })
            } else {
                json!({ "ok": false, "version": VERSION, "error": { "code": "E_RUNTIME", "message": err.to_string() } })
            }
        }
    }
}
"""
    if key == ("dotnet", "file_reader"):
        return """using System;
using System.Collections.Generic;
using System.IO;
using System.Text;
...
public static class Adapter {
  ...
  public static Dictionary<string, object> Handle(Dictionary<string, object> request) {
    ...
    if (!request.TryGetValue("source", out var sourceObj) || !string.Equals(sourceObj?.ToString(), "file", StringComparison.OrdinalIgnoreCase)) {
      return Error("E_UNSUPPORTED", "only file source supported");
    }
    if (!request.TryGetValue("path", out var pathObj) || string.IsNullOrEmpty(pathObj?.ToString())) {
      return Error("E_ARGS", "path must be a string");
    }
    var encodingName = request.TryGetValue("encoding", out var encObj) ? encObj?.ToString() : "utf-8";
    Encoding encoding;
    try {
      encoding = Encoding.GetEncoding(string.IsNullOrWhiteSpace(encodingName) ? "utf-8" : encodingName);
    } catch (Exception ex) {
      return Error("E_UNSUPPORTED", ex.Message);
    }
    var path = pathObj!.ToString();
    try {
      var content = File.ReadAllText(path, encoding);
      return Ok(new Dictionary<string, object> { { "content", content } });
