...
def generate_contract_docs(file_path: str, output_path: Optional[str] = None) -> str:
    ...
    if output_path:
        ...
        output_path = file_path.replace('.pw', '.md')
        with open(output_path, 'w') as f:
            f.write(docs)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python -m promptware.cli.generate_docs <contract.pw> [output.md]")
        sys.exit(1)
    ...
    output = output_path or file_path.replace('.pw', '.md')
