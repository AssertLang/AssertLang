...
import pytest
from language.parser import ALParseError, _parse_dsl, parse_al  # type: ignore[attr-defined]


def test_parse_al_basic_plan():
    text = """
lang python
start python app.py
file app.py:
...
def test_parse_dsl_requires_file():
    ...
    try:
        ...
        assert False, "Expected ALParseError"


def test_parse_al_fallback_prompt():
...
def test_parse_al_actions_without_files():
    text = """
tool http as fetch
...
def test_parse_al_parallel_branches():
    text = """
tool http as fetch
parallel:
  branch primary:
...
def test_parse_al_payload_list_index():
    text = """
tool http as fetch
...
def test_parse_al_reference_from_syntax():
    text = """
tool http as fetch
call fetch method=GET url="https://example.com"
...
def test_parse_al_reference_from_requires_alias():
    text = """
tool transform as xf
...
def test_parse_al_let_assignment():
    text = """
tool http as fetch
call fetch method=GET url="https://example.com"
...
def test_parse_al_fanout_case_labels():
    text = """
tool echo as send
...
def test_parse_al_inline_object_literal():
    text = """
tool logger as log
tool transform as xf
call log message="hi"
...
def test_parse_al_merge_with_custom_aliases():
    text = """
...
def test_parse_al_merge_modes():
    plan = parse_al("merge append into summary send.case").plan
    assert plan is not None
    action = plan["actions"][0]
    assert action["mode"] == "append"
    assert "append_key" not in action

    plan = parse_al("merge append items into summary send.case").plan
    assert plan is not None
    action = plan["actions"][0]
    assert action["mode"] == "append"
    assert action["append_key"] == "items"

    plan = parse_al("merge dict into summary send.case").plan
    assert plan is not None
    action = plan["actions"][0]
    assert action["mode"] == "dict"

    plan = parse_al("merge collect totals into summary send.case").plan
    assert plan is not None
    action = plan["actions"][0]
...
def test_parse_al_golden_fixtures(fixture: Path) -> None:
    source = fixture.with_name(fixture.name.replace(".plan.json", ".al"))
    expected = json.loads(fixture.read_text())
    prog = parse_al(source.read_text())
    assert prog.plan is not None
    assert prog.plan == expected


def test_parse_al_bad_indentation_sets_error_code():
    text = """
tool http as fetch
...
"""
    try:
        ...
        pytest.fail("expected ALParseError")
